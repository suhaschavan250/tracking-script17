(function () {
  ...
  function getConfigFromQuery() {
    ...
    const trackingScript = Array.from(scripts).find(s => s.src && s.src.includes('tracking-script17'));
    ...
    return {
      ...
      ctaMethod: params.get('ctaMatchMethod'), // Get the CTA method (Text or Button Id)
      ctaValue: params.get('ctaValue') || "" // Get the CTA value (text or ID)
    };
  }
  ...
  function handleClick(event) {
    ...
    console.log('Normalized Clicked:', normalizedClicked);

    sendToAllPlatforms('any_click');

    // Handle based on CTA method
    if (CONFIG.ctaMethod === 'CTA Text') {
      const matchedCTA = CONFIG.ctaTexts.some(text => text === normalizedClicked);
      if (matchedCTA) {
    ...
    if (CONFIG.ctaMethod === 'CTA Button Id') {
      ...
      console.log('Clicked Button ID:', elementId);

      // Check if any attribute contains 'id', 'ID', or 'Id'
      const idAttribute = Array.from(clickedElement.attributes).find(attr => /id/i.test(attr.name));
      
      if (idAttribute && idAttribute.value === CONFIG.ctaValue) {
